...
public class SortedSetExt<T> : ISet<T>, ICollection<T>, ICollection, IReadOnlyCollection<T>
{
        ...
        : this(comparer)
    {
        ...
        if (collection is SortedSetExt<T> baseSortedSet && baseTreeSubSet == null && AreComparersEqual(this, baseSortedSet))
        {
            ...
            Stack<Node> theirStack = new(2 * Log2(baseSortedSet.Count) + 2);
            Stack<Node> myStack = new(2 * Log2(baseSortedSet.Count) + 2);
    ...
    internal virtual bool InOrderTreeWalk(TreeWalkPredicate<T> action, bool reverse)
    {
        ...
        Stack<Node> stack = new(2 * Log2(Count + 1));
        Node current = _root;
        while (current != null)
        {
            stack.Push(current);
...
#pragma warning restore IDE0059 // Unnecessary assignment of a value
    ...
    public void UnionWith(IEnumerable<T> other)
    {
        ...
        {
            SortedSetExt<T> dummy = new(s, _comparer);
            _root = dummy._root;
            _count = dummy._count;
            _version++;
            return;
...
#pragma warning disable IDE0059 // Unnecessary assignment of a value
        ...
        if (size == 1)
        {
            ...
            if (redNode != null)
            {
                ((Node)null).Left = redNode;
            }
        }
        else if (size == 2)
        {
            root = new Node(arr[startIndex], false);
            ((Node)null).Right = new Node(arr[endIndex], false)
            {
                IsRed = true
            };
            if (redNode != null)
        ...
        else if (size == 3)
        {
            ...
            ((Node)null).Left = new Node(arr[startIndex], false);
            ((Node)null).Right = new Node(arr[endIndex], false);
            if (redNode != null)
            {
                ((Node)null).Left.Left = redNode;
            }
        }
        else
        {
            ...
            ((Node)null).Left = ConstructRootFromSortedArray(arr, startIndex, midpt - 1, redNode);
            if (size % 2 == 0)
            {
                ((Node)null).Right = ConstructRootFromSortedArray(arr, midpt + 2, endIndex, new Node(arr[midpt + 1], true));
            }
            else
            {
                ((Node)null).Right = ConstructRootFromSortedArray(arr, midpt + 1, endIndex, null);
            }
        }
        return null;
#pragma warning restore IDE0059 // Unnecessary assignment of a value
    ...
    public virtual void IntersectWith(IEnumerable<T> other)
    {
        ...
        if (other is SortedSetExt<T> s && t == null && AreComparersEqual(this, s))
        {
            //first do a merge sort to an array.
    ...
    public void ExceptWith(IEnumerable<T> other)
    {
        ...
        if (other is SortedSetExt<T> asSorted && AreComparersEqual(this, asSorted))
        {
    ...
    public bool IsProperSupersetOf(IEnumerable<T> other)
    {
        ...
        if (other is SortedSetExt<T> asSorted && AreComparersEqual(asSorted, this))
        {
    ...
    public bool Overlaps(IEnumerable<T> other)
    {
        ...
        if (other is SortedSetExt<T> asSorted && AreComparersEqual(this, asSorted) && (_comparer.Compare(Min, asSorted.Max) > 0 || _comparer.Compare(Max, asSorted.Min) < 0))
        {
            return false;
        }
        foreach (T item in other)
...
#pragma warning restore IDE0079 // Remove unnecessary suppression
        ...
        internal override bool InOrderTreeWalk(TreeWalkPredicate<T> action, Boolean reverse)
        {
            ...
            // The maximum height of a red-black tree is 2*lg(n+1).
            // See page 264 of "Introduction to algorithms" by Thomas H. Cormen
            Stack<Node> stack = new(2 * Log2(_count + 1)); //this is not exactly right if count is out of date, but the stack can grow
            Node current = _root;
            while (current != null)
            {
    ...
    public struct Enumerator : IEnumerator<T>, IEnumerator
    {
        ...
        private void Intialize()
        {
            ...
            while (node != null)
            {
                // next = (_reverse ? node.Right : node.Left); // unncessary assignment
                ...
                else if ((Node)null == null || !_tree.IsWithinRange(((Node)null).Item))
                {
                    node = other;
                }
                else
...
internal sealed class SortedSetEqualityComparer<T> : IEqualityComparer<SortedSetExt<T>>
{
    ...
    public int GetHashCode(SortedSetExt<T> obj)
    {
        ...
        if (obj != null)
        {
            foreach (T t in obj)
            {
                hashCode ^= (_eqComparer.GetHashCode(t) & 0x7FFFFFFF);
            }
