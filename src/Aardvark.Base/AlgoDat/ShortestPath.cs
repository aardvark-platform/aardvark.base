...
public class ShortestPath<T> : IShortestPath<T>
{
    ...
    public ShortestPath(List<T> nodes, List<(int, int)> edges, Func<T, T, float> getCostFunc)
    {
        ...
        for(int i = 0; i<m_nodes.Count; i++) m_neighbors[i] = [];
    ...
    private void CalculateByIndex(int index)
    {
        ...
        {
            [m_seedId] = activePixels.Insert(0, m_seedId)
        };
...
class FibonacciHeap<T>
{
    ...
    private void Consolidate()
    {
        ...
        do
        {
            ...
            {
                //var right = current.Right; // unnecessary variable 'right' if all uses below are unnecessary
                var first = current;
                var second = lookup[current.Degree];

                //// unncessary if statement if body consists of unnecessary assignment
                //if (second == right && right.Right != first) 
                //    right = right.Right; // unnecessary unsignment to 'right'
