...
public static class SubPrimitives
{
            ...
            this Polygon2f poly, ref int[] indices, float absoluteEpsilon)
    {
        ...
        int addFun(int i) => (i + 1) % count;
        int subFun(int i) => (i + count - 1) % count;
        ...
        for (int vi = 0; vi < count; vi++) // we possibly have to try all vertices
        {
            ...
            for (int pass = 0, goodPass = 0; pass - goodPass < 3; pass++, add = !add)
            {
                ...
                Func<int, int> nextFun = add ? addFun : subFun;
            ...
            int tvi, int[] tia, int[] pvibm, int[] tibm)
    {
        ...
        if (count < 3) throw new ArgumentOutOfRangeException(nameof(ia));
            ...
            V3f[] normalizedEdgeArray, bool[] straightArray)
    {
        ...
        if (straightArray == null) throw new ArgumentNullException(nameof(straightArray));
        if (count <= 2) throw new ArgumentException("Expected via.Length > 2.", nameof(via));
        if (straightArray.Length != count) throw new ArgumentException("Expected straightArray.Length == via.Length.", nameof(straightArray));
        if (normalizedEdgeArray != null && normalizedEdgeArray.Length != count) throw new ArgumentException("", nameof(normalizedEdgeArray));
