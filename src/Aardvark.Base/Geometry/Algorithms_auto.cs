...
namespace Aardvark.Base
{
    ...
    public static partial class GeometryFun
    {
        ...
        public static int[] Simplify(this V2f[] polyline, float epsilon)
        {
            ...
            if (epsilon < 0 || float.IsNaN(epsilon))
            ...
            int last = count - 1;
            int rootSplit = SimplifyFarthestPoint(polyline, 0, last, out float rootDistance);
            ...
            try
            {
                ...
                while (segmentCount > 0)
                {
                    ...
                    int indexMax = SimplifyFarthestPoint(polyline, indexFirst, indexLast, out float distanceMax);
        ...
        private static int SimplifyFarthestPoint(
            V2f[] polyline, int indexFirst, int indexLast, out float distanceMax)
        {
            var line = new Line2f(polyline[indexFirst], polyline[indexLast]);
            distanceMax = 0;
            int indexMax = indexFirst;
            for (int i = indexFirst + 1; i < indexLast; i++)
            {
                float distance = line.GetDistanceToLine(polyline[i]);
        ...
        public static int[] Simplify(this V2d[] polyline, double epsilon)
        {
            ...
            int last = count - 1;
            int rootSplit = SimplifyFarthestPoint(polyline, 0, last, out double rootDistance);
            if (rootDistance <= epsilon) return new[] { 0, last };
            ...
            try
            {
                ...
                while (segmentCount > 0)
                {
                    ...
                    int indexMax = SimplifyFarthestPoint(polyline, indexFirst, indexLast, out double distanceMax);
                    if (distanceMax > epsilon)
                    {
                        segments[segmentCount++] = ((long)indexMax << 32) | (uint)indexLast;
                        segments[segmentCount++] = ((long)indexFirst << 32) | (uint)indexMax;
        ...
        private static int SimplifyFarthestPoint(
            V2d[] polyline, int indexFirst, int indexLast, out double distanceMax)
        {
            var line = new Line2d(polyline[indexFirst], polyline[indexLast]);
            distanceMax = 0;
            int indexMax = indexFirst;
            for (int i = indexFirst + 1; i < indexLast; i++)
            {
                double distance = line.GetDistanceToLine(polyline[i]);
                if (distance > distanceMax)
                {
                    distanceMax = distance;
                    indexMax = i;
