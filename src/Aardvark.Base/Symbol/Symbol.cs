...
public readonly struct Symbol : IEquatable<Symbol>, IComparable<Symbol>, IComparable
{
    ...
    public override bool Equals(object obj) => (obj is Symbol symbol) && (Id == symbol.Id);
    ...
    {
        if (obj is Symbol symbol) return Id.CompareTo(symbol.Id);
    ...
    /// dictionary.
    /// </summary>
    public static Symbol operator -(Symbol symbol) => new(-symbol.Id);
...
public readonly struct TypedSymbol<T> : ITypedSymbol
{
    ...
    public static implicit operator TypedSymbol<T>(string str) => new(str);
...
public static class SymbolExtensions
{
    ...
    public static TypedSymbol<T> WithType<T>(this Symbol symbol) => new(symbol);
...
{
    private static readonly Dict<string, int> s_stringDict = new(1024);
    private static readonly Dict<Guid, int> s_guidDict = new(1024);
    private static readonly List<string> s_allStrings = new(1024);
    private static readonly List<Guid> s_allGuids = new(1024);
    private static SpinLock s_lock = new();
