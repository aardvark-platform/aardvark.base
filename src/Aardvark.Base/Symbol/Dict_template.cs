...
#pragma warning disable CA1512 // Member name can be simplified.
    ...
    //# }
{
    ...
    public __uitype__ Capacity
    {
        ...
        set
        {
            ...
            //# } else {
            if (value < m_count)
                throw new ArgumentOutOfRangeException(nameof(value), "The new capacity is less than the current number of elements.");
...
#pragma warning restore IDE0034 // Simplify 'default' expression
            ...
            if (hasHashPar) { */__itype__ hash, /*# } */out TValue value)
    {
        ...
        var fi = ((__uitype__)hash) % m_capacity;
        var ei = m_firstArray[fi].Next;
        if (ei == 0) { value = default; return false; }
        if (m_firstArray[fi].Item__Hash__ == hash/*# if (hasKey) { */
            && key.Equals(m_firstArray[fi].Item__Key__)/*# } */)
        {
    ...
    public bool Remove(__tkey__ key/*# if (hasHashPar) { */, __itype__ hash/*# } */)
    {
        ...
        Monitor.Enter(this); try
        {
            return m_dict.TryRemove(key/*# if (hasHashPar) { */, hash/*# } */, out var value);
        }
        finally { Monitor.Exit(this); }
        //# } else { // !wrapped
        return TryRemove(key/*# if (hasHashPar) { */, hash/*# } */, out _);
        //# } // !wrapped
    }
...
#pragma warning restore IDE0305 // Collection initialization can be simplified.
    ...
    {
        if (array is /*# if (hasValue) { */KeyValuePair</*# } */__tkey__/*# if (hasValue) { */, TValue>/*# } */[] typedArray)
    ...
    public bool TryGetValue<TType>(TypedSymbol<TType> key, out TType value)
        where TType : TValue
    {
        if (TryGetValue(key.Symbol, out var val)) { value = (TType)val; return true; }
        value = default;
        return false;
    }
    ...
    public TType GetAs<TType>(__tkey__ key, TType defaultValue)
        where TType : TValue
    {
        if (TryGetValue(key, out var value)) return (TType)value;
        return defaultValue;
    }
