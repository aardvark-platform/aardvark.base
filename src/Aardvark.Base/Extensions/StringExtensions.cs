...
public static class StringFun
{
    ...
    public static bool ToBool(this string self)
    {
        ...
        _ = double.TryParse(s, out double r);
        return r != 0.0;
    }
    ...
    {
        if (!decimal.TryParse(self, out decimal result))
        {
            result = def;
        }
...
#else
        return string.Concat(self[..1].ToUpperInvariant(), self.AsSpan(1));
#endif
    }
    ...
    static readonly Dictionary<string, string> s_irregularPlural = new()
    {
        { "aircraft", "aircraft" },
        { "axis", "axes" },
        { "bison", "bison" },
    ...
    {
        if (s == 1) return word;
        if (s_irregularPlural.TryGetValue(word, out string irregular))
            return irregular;
        else
        {
            for (int i = s_pluralEndings.Length - 1; i > 0; i--)
            {
                ...
                if (s_pluralEndings[i].TryGetValue(ending, out string plural))
                    return word.Substring(0, word.Length - i) + plural;
#else
                string ending = word[^i..];
                if (s_pluralEndings[i].TryGetValue(ending, out string plural))
                    return word[..^i] + plural;
#endif
            }
            return word + "s";
        }
...
#else
        return start < end ? self[start..end] : null;
#endif
    }
...
#else
                        yield return text[begin..pos].Trim();
#endif
                    break;
                case ',':
                    if (level == 1)
