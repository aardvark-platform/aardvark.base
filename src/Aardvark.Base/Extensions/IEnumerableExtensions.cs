...
namespace Aardvark.Base
{
    ...
    public static partial class EnumerableEx
    {
        ...
        /// Each selector is evaluated exactly once per source element.
        /// </summary>
        ...
        /// <param name="duplicateValue">The value stored when a duplicate key is encountered.</param>
        ...
        /// Each selector is evaluated exactly once per source element. For duplicate keys,
        /// <paramref name="duplicateKeyKeepElement"/> decides whether the existing or incoming value is stored.
        /// </summary>
        /// <typeparam name="TSource">The type of the elements of source.</typeparam>
        /// <typeparam name="TKey">The type of the key returned by keySelector.</typeparam>
        /// <typeparam name="TElement">The type of the value returned by elementSelector.</typeparam>
        ...
        /// <param name="keySelector">A function to extract a key from each element.</param>
        /// <param name="elementSelector">A transform function to produce a result element value from each element.</param>
        /// <param name="duplicateKeyKeepElement">Called with the existing value first and the incoming value second. Return true to keep the existing value, or false to replace it with the incoming value.</param>
            ...
            Func<TElement, TElement, bool> duplicateKeyKeepElement)
        {
            ...
            foreach (var v in source)
            {
                ...
                {
                    if (!duplicateKeyKeepElement(existing, element))
        ...
        /// containing all Key Value pairs with distinct keys.
        /// Each selector is evaluated exactly once per source element. For duplicate keys,
        /// the value returned by <paramref name="duplicateKeyElementSelector"/> is stored.
        /// </summary>
        /// <typeparam name="TSource">The type of the elements of source.</typeparam>
        /// <typeparam name="TKey">The type of the key returned by keySelector.</typeparam>
        /// <typeparam name="TElement">The type of the value returned by elementSelector.</typeparam>
        ...
        /// <param name="keySelector">A function to extract a key from each element.</param>
        /// <param name="elementSelector">A transform function to produce a result element value from each element.</param>
        /// <param name="duplicateKeyElementSelector">Called with the existing value first and the incoming value second. The returned value replaces the existing value.</param>
        /// <returns>A System.Collections.Generic.Dictionary(of TKey,TValue) that contains values of type TElement selected from the input sequence.</returns>
        /// <exception cref="ArgumentNullException">Any of the arguments is null. -or- keySelector produces a key that is null.</exception>
        public static Dictionary<TKey, TElement> ToDictionaryDistinct<TSource, TKey, TElement>(
            this IEnumerable<TSource> source,
            ...
            )
        {
            ...
            foreach (var v in source)
            {
                ...
                var element = elementSelector(v);
                if (dictionary.TryGetValue(key, out var existing))
                    dictionary[key] = duplicateKeyElementSelector(existing, element);
                else
                    dictionary.Add(key, element);
            }
            return dictionary;
