...
public static partial class EnumerableEx
{
    ...
    public static T[] TakeToArray<T>(this IEnumerable<T> self, int count)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            if (en.MoveNext())
                ...
                throw new ArgumentException("Less than 'count' elements.");
        }
        return array;
    }
        ...
        params Func<TElement, object>[] groupSelectorFuncs)
    {
        ...
        {
            if (selectorFunc is null) throw new ArgumentNullException(nameof(groupSelectorFuncs));
            result = from g in
    ...
    /// </summary>
    public static IEnumerable<T> NonNull<T>(IEnumerable<T> elements) => elements ?? [];
    ...
    public static TSrc MaxElement<TSrc, TDst>(this IEnumerable<TSrc> source, Func<TSrc, TDst> selector, Func<TDst, TDst, bool> greaterThan, TDst minVal, TSrc defaultRv, out TDst rv_maxVal)
    {
        ...
        if (greaterThan == null) throw new ArgumentNullException(nameof(greaterThan));
    ...
    public static TSrc MinElement<TSrc, TDst>(this IEnumerable<TSrc> source, Func<TSrc, TDst> selector, Func<TDst, TDst, bool> smallerThan, TDst maxVal, TSrc defaultRv, out TDst rv_minVal)
    {
        if (source == null) throw new ArgumentNullException(nameof(source));
        if (selector == null) throw new ArgumentNullException(nameof(selector));
        if (smallerThan == null) throw new ArgumentNullException(nameof(smallerThan));
        var e = source.GetEnumerator();
    ...
    public static int MinIndex<T>(this IEnumerable<T> self)
       where T : IComparable<T>
    {
        return MinIndex(self, (a, b) => a.CompareTo(b) < 0, out T foo);
    }

    public static int MaxIndex<T>(this IEnumerable<T> self)
        where T : IComparable<T>
    {
        return MaxIndex(self, (a, b) => a.CompareTo(b) > 0, out T foo);
    }
    ...
    {
        if (self == null || self.Length == 0) throw new ArgumentException("Empty collection.");
    ...
    {
        if (self == null || self.Count == 0) throw new ArgumentException("Empty collection.");
