...
public partial struct Rot3f : IEquatable<Rot3f>
{
    ...
    public static QuaternionF operator +(Rot3f r, QuaternionF q)
        => new(r.W + q.W, r.X + q.X, r.Y + q.Y, r.Z + q.Z);
    ...
    public static QuaternionF operator +(QuaternionF q, Rot3f r)
        => new(q.W + r.W, q.X + r.X, q.Y + r.Y, q.Z + r.Z);
    ...
    public static Rot3f FromAngleAxis(V3f angleAxis)
    {
        ...
        if (theta2 > Constant<float>.PositiveTinyValue)
        {
            ...
            return new(Fun.Cos(thetaHalf), k * angleAxis);
        }
        else
            return new(1, 0, 0, 0);
    }
    ...
    public static Rot3f FromM33f(M33f m, float epsilon = (float)1e-6)
    {
        ...
        if (tr > 0)
        {
            ...
            float w = s / 4;
            return new(new QuaternionF(w, x, y, z).Normalized);
        }
    ...
    public static Rot3f Rotation(V3f normalizedAxis, float angleInRadians)
    {
        ...
        return new(halfAngle.Cos(), normalizedAxis * halfAngleSin);
    }
    ...
    public static Rot3f RotateInto(V3f from, V3f into)
    {
        ...
        if (d.ApproximateEquals(-1))
            return new(0, from.AxisAlignedNormal());
        else
        {
            var q = new QuaternionF(d + 1, Vec.Cross(from, into));
            return new(q.Normalized);
        }
    }
    ...
    {
        var halfAngle = angleInRadians / 2;
        return new(halfAngle.Cos(), new V3f(halfAngle.Sin(), 0, 0));
    }
    ...
    {
        var halfAngle = angleInRadians / 2;
        return new(halfAngle.Cos(), new V3f(0, halfAngle.Sin(), 0));
    }
    ...
    {
        var halfAngle = angleInRadians / 2;
        return new(halfAngle.Cos(), new V3f(0, 0, halfAngle.Sin()));
    }
    ...
    public static explicit operator Trafo3f(Rot3f r)
        => new((M44f)r, (M44f)r.Inverse);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static explicit operator Rot3d(Rot3f r)
        => new((double)r.W, (double)r.X, (double)r.Y, (double)r.Z);
    ...
    public override readonly bool Equals(object other)
        => (other is Rot3f o) && Equals(o);

    public override readonly string ToString()
        => string.Format(CultureInfo.InvariantCulture, "[{0}, {1}]", W, V);

    public static Rot3f Parse(string s)
    {
        var x = s.NestedBracketSplitLevelOne().ToArray();
        return new(float.Parse(x[0], CultureInfo.InvariantCulture), V3f.Parse(x[1]));
    }
...
public partial struct Rot3d : IEquatable<Rot3d>
{
    ...
    public static Rot3d FromM33d(M33d m, double epsilon = (double)1e-6)
    {
        ...
        if (tr > 0)
        {
            ...
            double w = s / 4;
            return new(new QuaternionD(w, x, y, z).Normalized);
        }
        else if (m.M00 > m.M11 && m.M00 > m.M22)
        {
    ...
    public static Rot3d RotateInto(V3d from, V3d into)
    {
        ...
        {
            var q = new QuaternionD(d + 1, Vec.Cross(from, into));
            return new(q.Normalized);
        }
    }
    ...
    {
        var halfAngle = angleInRadians / 2;
        return new(halfAngle.Cos(), new V3d(halfAngle.Sin(), 0, 0));
    }
    ...
    {
        var halfAngle = angleInRadians / 2;
        return new(halfAngle.Cos(), new V3d(0, halfAngle.Sin(), 0));
    }
    ...
    {
        var halfAngle = angleInRadians / 2;
        return new(halfAngle.Cos(), new V3d(0, 0, halfAngle.Sin()));
    }
    ...
    public static explicit operator Trafo3d(Rot3d r)
        => new((M44d)r, (M44d)r.Inverse);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static explicit operator Rot3f(Rot3d r)
        => new((float)r.W, (float)r.X, (float)r.Y, (float)r.Z);
    ...
    {
        var x = s.NestedBracketSplitLevelOne().ToArray();
        return new(double.Parse(x[0], CultureInfo.InvariantCulture), V3d.Parse(x[1]));
    }
