...
[Serializable]
public abstract partial class PixImage(Col.Format format) : IPix
{
    public Col.Format Format = format;
    ...
    private class PgmPixLoader : IPixLoader
    {
        ...
        public void SaveToStream(Stream stream, PixImage image, PixSaveParams saveParams)
        {
            ...
            var sw = new StreamWriter(stream, Encoding.ASCII) { NewLine = "\n" };
            sw.WriteLine("P5");
    ...
    private static readonly Dictionary<IPixLoader, int> s_loaders = [];
...
public partial class PixImage<T> : PixImage
{
    ...
    public PixImage(string filename, IPixLoader loader = null)
    {
        ...
        if (loadImage is not PixImage<T> image || image.ChannelCount != channelCount)
    ...
    /// green, blue, (alpha).
    /// </summary>
    public Matrix<T>[] ChannelArray => [.. Channels];
    ...
    public PixImage<T> Transformed(ImageTrafo trafo)
        => new(Format, Volume.Transformed(trafo));
