...
module internal QRHelpers =
    ...
    let inline qrBidiagonalizeNative (U : NativeMatrix<float>) (B : NativeMatrix<float>) (Vt : NativeMatrix<float>) =
        let rows = int B.SY
        let cols = int B.SX
...
type QR private() =
    ...
    static member BidiagonalizeInPlace(U : NativeMatrix<float>, B : NativeMatrix<float>, Vt : NativeMatrix<float>) =
        qrBidiagonalizeNative U B Vt |> ignore
    ...
    static member BidiagonalizeInPlace(U : Matrix<float>, B : Matrix<float>, Vt : Matrix<float>) =
        ...
        tensor {
            ...
            let! pB = &B
            let! pVt = &Vt
            return qrBidiagonalizeNative pU pB pVt |> ignore
        }
