...
{
    public class IllegalSizeException(string message) : Exception(message)
    {
    };
...
internal class Converter
{
    ...
    /// </summary>
    public static readonly Converter Global = new();
    ...
    private readonly Dictionary<(string, string), Action<Convertible, Convertible>> m_actionMap = [];

    private readonly Dictionary<(string, string), Annotation[]>             m_annotationsMap = [];

    private readonly Dictionary<string, Dictionary<string, RoutingEntry>>   m_routingMap = [];

    private readonly Dictionary<(string, string), int>                      m_weightMap = [];

    private readonly Dictionary<string, List<Func<string, Convertible>>>    m_fileExtensionsMap = [];

    private readonly Dictionary<string, List<Func<Convertible>>>            m_rawConvertiblesMap = [];

    private readonly Dictionary<string, List<Func<Convertible>>>            m_creatorsMap = [];

    private readonly Dictionary<string, List<string>>                       m_resourcesMap = [];

    private readonly List<string>                                           m_availableResources = [];
    ...
    public Converter(Converter converter)
    {
        ...
        try
        {
            ...
            this.m_availableResources = [.. converter.m_availableResources];
    ...
    /// </summary>
    private static Dictionary<string, List<T>> Clone<T>(Dictionary<string, List<T>> dictionary)
    {
    ...
    {
        Convert(source, target, CloneRoutingTable(), []);
    }
        ...
        List<Convertible> tempConvertibles)
    {
        ...
        try
        {
            ...
            if (distance == 1 || distance == c_distanceComplex)
            {
                ...
                if (tempConvertibles != null)
                {
                    ...
                    foreach (var temp in tempConvertibles)
                    {
                        ...
                        {
                            if (temp.Data is IDisposable tempDisposable)
            ...
            else if (distance < c_distanceUnreachable)
            {
                ...
                {
                    var tmp = new Convertible(routingMap[sourceDescr][targetDescr].Next, null);
    ...
    private void DirectConvert(Convertible source, Convertible target)
    {
        ...
        if (m_actionMap.TryGetValue(conversion, out Action<Convertible, Convertible> procedure))
    ...
    public void RegisterRaw(string descriptor, Func<Convertible> creator)
    {
        ...
        try
        {
            ...
            if (!m_rawConvertiblesMap.ContainsKey(descriptor))
                m_rawConvertiblesMap[descriptor] = [];
    ...
    public void RegisterExtension(string fileExtension, Func<string, Convertible> creator)
    {
        ...
        try
        {
            ...
            if (!m_fileExtensionsMap.ContainsKey(fileExtension))
                m_fileExtensionsMap[fileExtension] = [];
    ...
    public void RegisterResource(string resource, List<string> descriptors)
    {
        ...
        try
        {
            ...
            if (!m_resourcesMap.TryGetValue(resource, out List<string> value))
                m_resourcesMap[resource] = descriptors;
            else
                value.AddRange(descriptors);
        ...
        IEnumerable<Annotation> annotations)
    {
        ...
        m_annotationsMap[(sourceDescriptor, targetDescriptor)] = [.. annotations];
    }
    ...
    private void AddNewRoutingObject(String descriptor)
    {
        ...
        Dictionary<string, RoutingEntry> newRoutingTable = [];
    ...
    {
        if (!m_creatorsMap.TryGetValue(descriptor, out List<Func<Convertible>> creators))
            return null;
    ...
    public Convertible FindMatchingRaw(Convertible c)
    {
        ...
        if (!m_rawConvertiblesMap.TryGetValue(desc, out List<Func<Convertible>> creators))
            return null;
    ...
    {
        if (!m_rawConvertiblesMap.TryGetValue(conv.Descriptor, out List<Func<Convertible>> creators))
            return null;
    ...
    public Convertible FindConvertibleByFileName(string fileName)
    {
        ...
        if (!m_fileExtensionsMap.TryGetValue(fileExt, out List<Func<string, Convertible>> creators))
        ...
        string sourceDescription, string targetDescription)
    {
        ...
        if (!m_routingMap.TryGetValue(sourceDescription, out Dictionary<string, RoutingEntry> value)) throw new ArgumentException("unknow convertible");
        var tmp = value;
    ...
    public IEnumerable<Annotation> GetAnnotations(string source, string target)
    {
        ...
        if (m_annotationsMap.TryGetValue(key, out Annotation[] value)) return value;
        return [];
    }
...
internal class ConversionNotPossibleException(
    string source, string target, string text
    ) : Exception(string.Format("unable to convert from \"{0}\" to \"{1}\": {2}", source, target, text))
{
}
