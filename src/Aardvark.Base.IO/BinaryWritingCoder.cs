    ...
    : BaseWritingCoder, IWritingCoder, IDisposable
{
    ...
    private bool HandleRef(ref object obj)
    {
        if (m_doRefs)
        {
            if (c_coderVersion < 5)
            {
                if (m_refs.TryGetValue(obj, out int index))
                {
    ...
    public int CodeCount<T>(ref T value, Func<T, int> counter) where T : class
    {
        ...
        if (m_refs.TryGetValue(value, out int refNum))
        {
    ...
    private long[] CodeCountLong<T>(ref T[,] array)
    {
        return CodeCountArray(ref array, v => [v.GetLongLength(0), v.GetLongLength(1) ]);
    }

    private long[] CodeCountLong<T>(ref T[,,] array)
    {
        return CodeCountArray(ref array, v => [v.GetLongLength(0), v.GetLongLength(1), v.GetLongLength(2) ]);
    }
    ...
    private static long[] LengthArray(Array array)
    {
        ...
        {
            1 => array.LongLength.IntoArray(),
            2 => [array.GetLongLength(0), array.GetLongLength(1)],
            3 => [array.GetLongLength(0), array.GetLongLength(1), array.GetLongLength(2)],
            _ => throw new ArgumentException("cannot code arrays with more than 3 dimensions"),
        };
    }
