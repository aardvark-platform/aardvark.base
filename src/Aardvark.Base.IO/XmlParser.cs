...
public class XmlItem(Symbol name, params XmlAtt[] atts) : IXml
{
    public Symbol Name = name;
    public List<XmlAtt> AttList = [.. atts];
    ...
    public T FirstSub<T>(out int index)
        where T : class
    {
        if ((index = NextSubIndex(out T item)) < 0) return null;
        return item;
    }
    ...
    public T NextSub<T>(ref int index)
        where T : class
    {
        if ((index = NextSubIndex(out T item, index)) < 0) return null;
        return item;
    }
        ...
        where T : class
    {
        ...
        if (SubList[0] is not T sub) throw new ArgumentException();
        return sub;
    }
        ...
        where T : class
    {
        ...
        for (int c = SubList.Count; i < c; i++)
        {
            if (SubList[i] is T node) { item = node; return i; }
        }
        item = default;
        return -1;
    }
...
public class XmlDirectText(Text text) : IXml, IXmlText
{
...
public class XmlLazyText(Action<StreamWriter> writeAct) : IXml, IXmlText
{
    public Action<StreamWriter> WriteAct = writeAct;
...
public class XmlDoc : XmlItem, IXml
{
    ...
    public new void WriteTo(StreamWriter writer, int indent = 0)
    {
        ...
        foreach (var xml in SubList)
        {
            if (xml is XmlDirectText text && text.Text.IsWhiteSpace) continue;
...
public class XmlParser : TextParser<XmlParser>
{
    ...
    public static readonly Rx SymbolRx = new(@"[A-Za-z_][0-9A-Za-z_]*");

    public static readonly State<XmlParser, XmlItem> TagState = new(new[] {
